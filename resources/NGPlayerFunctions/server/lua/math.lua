...
MathTimer = setTimer ( createMathProblem, 600000, 0 )
...
addCommandHandler ( "result", function ( p, cmd, answer )
	if ( isMathQuestion ) then
		if ( tonumber ( answer ) == tonumber ( mathTable.answer ) ) then
			...
			outputDebugString ( "MATH: "..getPlayerName ( p ).." attempted to answer "..mathTable.question.." with an answer of "..mathTable.answer.." for a reward of $"..mathTable.reward.." || Result: True", 0, 0, 255, 0 )
			...
			outputDebugString ( "MATH: "..getPlayerName ( p ).." attempted to answer "..mathTable.question.." with an answer of "..mathTable.answer.." for a reward of $"..mathTable.reward.." || Result: False", 0, 255, 0, 0 )
			return false
		end
	else
		outputChatBox ( "#DF743F[MATH QUESTION] #ffffffThere arn't any questions availble right now. Try again later.", p, 255, 0, 0, true )
		outputDebugString ( "MATH: "..getPlayerName ( p ).." attempted to answer [NIL] with an answer of "..answer.." for a reward of $[NIL] || Result: No Question", 0, 255, 255, 0 )
		return false
	end
end )