...
function onSpawnerHit ( p )
	...
	if ( getElementType ( p ) == 'player' and not isPedInVehicle ( p ) ) then
		triggerClientEvent ( p, 'NGSpawners:ShowClientSpawner', p, getElementData ( source, "NGVehicles:SpawnVehicleList" ), source )
	end
end
...
addEventHandler ( "NGSpawners:spawnVehicle", root, function ( id, x, y, z, rz, job ) 
	exports['NGAntiRestart']:createPlayerVehicle ( source, id, x, y, z, rz, true, job )
end )
