...
local Vehicles = guiCreateWindow( ( sx - 470 ), ( sy - 344 ), 466, 340, "Vehicles", false)
local cars = guiCreateGridList(11, 28, 274, 301, false, Vehicles)
option['show'] = guiCreateButton(295, 28, 162, 53, "Show", false, Vehicles)
option['recover'] = guiCreateButton(295, 87, 162, 53, "Recover", false, Vehicles)
option['sell'] = guiCreateButton(295, 150, 162, 53, "Sell", false, Vehicles)
option['give'] = guiCreateButton(295, 213, 162, 53, "Give", false, Vehicles)
option['close'] = guiCreateButton(295, 276, 162, 53, "Close", false, Vehicles)  
...
addEventHandler ( "NGVehicles:onServerSendClientVehicleList", root, function  ( cList ) 
	...
	if ( #cList == 0 ) then
		...
		for i, v in ipairs ( cList ) do 
			...
			guiGridListSetItemText ( cars, row, 1, tostring ( i ), true, true )
