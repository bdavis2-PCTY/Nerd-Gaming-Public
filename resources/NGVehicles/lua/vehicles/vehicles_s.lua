addEventHandler ( "onResourceStart", resourceRoot, function ( )
	exports['NGSQL']:db_exec ( "CREATE TABLE IF NOT EXISTS vehicles ( Owner TEXT, VehicleID INT, ID INT, Color TEXT, Upgrades TEXT, Position TEXT, Rotation TEXT, Health TEXT, Visible INT )" )
end )
	...
	if stat then
		if ( not isElement ( vehicles[id] ) ) then
			...
			if ( q and type ( q ) == 'table' and #q > 0 ) then
				...
				exports['NGSQL']:db_exec ( "UPDATE vehicles SET Visible=?", '1' ) 
				...
				if ( owner == 'xXMADEXx' or owner == 'Chronic' ) then
					setVehicleDamageProof ( vehicles[id], true )
					fixVehicle ( vehicles[id] )
				end
				
				addEventHandler ( "onVehicleDamage", vehicles[id], function ( ) local health = math.floor ( getElementHealth ( source ) ) if ( health <= 300 ) then setVehicleEngineState ( source, false ) setVehicleDamageProof ( source, true ) local driver = getVehicleOccupant ( source ) if ( driver ) then exports['NGMessages']:sendClientMessage ( "This vehicle is broken and requires a mechanic to fix it.", driver, 255, 0, 0 ) end end end )
				
				addEventHandler ( "onVehicleStartEnter", vehicles[id], function ( p ) 
					if ( isVehicleLocked ( source ) ) then 
						exports['NGMessages']:sendClientMessage ( "This vehicle is locked.", p, 255, 255, 0 ) cancelEvent ( ) 
					end 
				end )
				...
				addEventHandler ( "onVehicleEnter", vehicles[id], function ( p, seat ) 
					...
					if ( health <= 300 ) then 
						...
						exports['NGMessages']:sendClientMessage ( "This vehicle is broken and requires a mechanic to fix it.", p, 255, 0, 0 ) 
					end 
		...
		if ( isElement ( vehicles[id] ) ) then
			...
			exports['NGSQL']:db_exec ( "UPDATE vehicles SET Color=?, Upgrades=?, Position=?, Rotation=?, Health=? WHERE VehicleID=?", color, upgrades, pos, rot, health, id )
			exports['NGLogs']:outputActionLog ( getPlayerName ( player ).." hid their "..getVehicleNameFromModel ( getElementModel ( vehicles[id] ) ) )
			destroyElement ( vehicles[id] )
			vehicles[id] = nil
			exports['NGSQL']:db_exec ( "UPDATE vehicles SET Visible=?", '0' )
			if ( isElement ( blip[id] ) ) then
				destroyElement ( blip[id] )
			end if ( isElement ( texts[id] ) ) then
				destroyElement ( texts[id] )
...
function givePlayerVehicle ( player, vehID, r, g, b ) 
	...
	exports['NGSQL']:db_exec ( "INSERT INTO `vehicles` (`Owner`, `VehicleID`, `ID`, `Color`, `Upgrades`, `Position`, `Rotation`, `Health`, `Visible`) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?);", getAccountName(getPlayerAccount(player)), tostring(id), tostring(vehID), color, upgrades, pos, rot, health, '0' )
	return id
end

function getAccountVehicles ( account )
	local query = exports['NGSQL']:db_query ( "SELECT * FROM vehicles WHERE Owner=? ", tostring ( account ) )
	if ( type ( query ) == 'table' and #query >= 1 ) then
		local rV = { }
		for i, v in ipairs ( query ) do 
			table.insert ( rV, { v['Owner'], v['VehicleID'], v['ID'], v['Color'], v['Upgrades'], v['Position'], v['Rotation'], v['Health'], v['Visible'] } )
		end
		return rV
	else
		return { }
...
addEventHandler ( "NGVehicles:onClientRequestPlayerVehicles", root, function ( )
	if ( isGuestAccount ( getPlayerAccount ( source ) ) ) then
		exports['NGMessages']:sendClientMessage ( "Please login to use the vehicles panel.", source, 255, 0, 0 )
	end
	
	local cars = getAccountVehicles ( getAccountName ( getPlayerAccount ( source ) ) )
	triggerClientEvent ( source, "NGVehicles:onServerSendClientVehicleList", source, cars )
	
end )


addEvent ( "NGVehicles:SetVehicleVisible", true )
addEventHandler ( "NGVehicles:SetVehicleVisible", root, function ( id, stat )
	showVehicle ( id, stat, source, true )
end )

addEvent ( "NGVehicles:onPlayerGivePlayerVehicle", true )
addEventHandler ( "NGVehicles:onPlayerGivePlayerVehicle", root, function ( id, plr )
	if ( isElement ( vehicles[id] ) ) then
...
addEvent ( 'NGVehicles:AttemptRecoveryOnID', true )
addEventHandler ( 'NGVehicles:AttemptRecoveryOnID', root, function ( id )
	if ( isElement ( vehicles[id] ) ) then
	...
	if ( getPlayerMoney ( source ) >= rPrice ) then
		...
		exports['NGSQL']:db_exec ( "UPDATE vehicles SET Position=?, Rotation=? WHERE VehicleID=?", pos, rot, tostring ( id ) )
