...
local font = { face = "default", size = 1.3, maxwidth = 300 }
...
function tooltipShow()
  ...
  if getElementData(source, "tooltip-text") and getElementType(source):find("gui-", 1, true) then
	...
	setElementData(source,"isHoveringOnGUIElement",true)
	...
	if ( getElementData ( source, "tooltip-displaytext" )==false ) then
		return
	end
    ...
    tooltips[source].y = math.floor(y - tooltips[source].h - 12) - 5
...
function tooltipHide(element)
  ...
  if tooltips[e] then 
	setElementData(e,"isHoveringOnGUIElement",false)
    tooltips[e].ticks = 10
    tooltips[e].step = -1
  end
end  
