...
function dxText:extent()
	local extent = dxGetTextWidth ( self.strText, self.fScale, "default-bold" )
...
function dxText:height()
	local height = dxGetFontHeight ( self,fScale, "default-bold" )
	if self.strType == "stroke" or self.strType == "border" then
...
function dxText:font(font)
	if not validFonts[font] then return "default-bold" end
	self.strFont = "default-bold"
	return true
end
		...
		for self,_ in pairs(visibleText) do
			while true do
						...
						for offsetX=-outlinesize,outlinesize,outlinesize do
							for offsetY=-outlinesize,outlinesize,outlinesize do
								if not (offsetX == 0 and offsetY == 0) then
									dxDrawText(self.strText, l + offsetX, t + offsetY, r + offsetX, b + offsetY, tocolor(att2, att3, att4, att5), self.fScale, "default-bold", self.bHorizontalAlign, self.bVerticalAlign, self.bClip, self.bWordWrap, self.bPostGUI )
								end
							end
						end
					end
				elseif type == "shadow" then
					...
					att4 = att4 or 0
					att5 = att5 or self.tColor[4]
					dxDrawText(self.strText, l + shadowDist, t + shadowDist, r + shadowDist, b + shadowDist, tocolor(att2, att3, att4, att5), self.fScale, "default-bold", self.bHorizontalAlign, self.bVerticalAlign, self.bClip, self.bWordWrap, self.bPostGUI )
				end
				dxDrawText ( self.strText, l, t, r, b, tocolor(unpack(self.tColor)), self.fScale, "default-bold", self.bHorizontalAlign, self.bVerticalAlign, self.bClip, self.bWordWrap, self.bPostGUI )
				break
			end
		end
	end
